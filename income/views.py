...
# Calculate vat and tax of incomes


def claculate_tax(incoms):
    for income in Income.objects.all():
        income.tax_amount = round(
            income.gross_amount * decimal.Decimal(0.03), 0)
...
def get_season_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        if request.POST['season'] == '1':
            incoms = Income.objects.filter(
                month__in=[1, 2, 3]).filter(year=1398)
        elif request.POST['season'] == '2':
            incoms = Income.objects.filter(
                month__in=[4, 5, 6]).filter(year=1398)
        elif request.POST['season'] == '3':
            incoms = Income.objects.filter(
                month__in=[7, 8, 9]).filter(year=1398)
        elif request.POST['season'] == '4':
            incoms = Income.objects.filter(
                month__in=[10, 11, 12]).filter(year=1398)
...
def get_employer_payment_view(request, *args, **kwargs):
    if request.method == 'POST':
        ...
        total_income = Income.objects.filter(
            employer_id=empolyer).aggregate(Sum('gross_amount'))
        total_tax = Income.objects.filter(
            employer_id=empolyer).aggregate(Sum('tax_amount'))
        total_VAT = Income.objects.filter(
            employer_id=empolyer).aggregate(Sum('VAT_amount'))
        total_pay = Income.objects.filter(
            employer_id=empolyer).aggregate(Sum('pay_amount'))
        view = ViewGenerator(table=Income, entities=incoms, opration_buttons={},
...
def get_contracts_view(request, id, *args, **kwargs):
    ...
    for contract in contracts:
        ...
        contract.progress = '{0:.3g}'.format(
            total_income['gross_amount__sum'] / contract_amount)
        contract.save()
...
def get_contract_payments_view(request, *args, **kwargs):
    if request.method == 'POST':
        ...
        total_income = Income.objects.filter(
            contract_id=contract).aggregate(Sum('gross_amount'))
        total_tax = Income.objects.filter(
            contract_id=contract).aggregate(Sum('tax_amount'))
        total_VAT = Income.objects.filter(
            contract_id=contract).aggregate(Sum('VAT_amount'))
        total_pay = Income.objects.filter(
            contract_id=contract).aggregate(Sum('pay_amount'))
        view = ViewGenerator(table=Income, entities=incoms, opration_buttons={},
                             select_checkbox=False, add_url='income-create', ordering=ordering)
        context = view.get_context_template()
        additon = {'form': form}
